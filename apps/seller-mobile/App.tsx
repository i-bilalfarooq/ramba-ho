...
interface BiddingRequest {
  ...
  model: string;
  year: string;
  status: 'searching' | 'quoted' | 'accepted' | 'declined' | 'quotes_received';
  quotes: Quote[];
}
...
export default function App() {
  ...
  const [activeRequests, setActiveRequests] = useState<BiddingRequest[]>([]);

  // Polling for real-time updates from mock server
  useEffect(() => {
  ...
  const handleSendQuote = async () => {
    ...
    try {
      await fetch(`${API_URL}/quotes`, {
        ...
        body: JSON.stringify({ requestId: selectedRequest.id, quote: newQuote })
      });
      setSelectedRequest(null);
      setQuoteData({ price: '', condition: 'new' });
      alert('Quote sent successfully!');
    } catch (e) {
      alert('Error sending quote. Make sure mock server is running.');
    }
  };
